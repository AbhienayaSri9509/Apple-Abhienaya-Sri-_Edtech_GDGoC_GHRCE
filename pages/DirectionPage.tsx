...
const DirectionPage: React.FC = () => {
    ...
    useEffect(() => {
        if (!GOOGLE_MAPS_API_KEY) {
            console.warn('Missing VITE_GOOGLE_MAPS_API_KEY. Using Mock Mode.');
            // Mock map initialization or placeholder
            if (mapRef.current) {
                mapRef.current.innerHTML = `
                    <div style="width:100%;height:100%;background:#222;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#888;text-align:center;">
                        <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mb-4"><polygon points="3 11 22 2 13 21 11 13 3 11"></polygon></svg>
                        <p class="font-bold text-white mb-2">Map Demo Mode</p>
                        <p>Google Maps API Key is missing.</p>
                        <p>This is a placeholder for the map directions.</p>
                    </div>
                `;
            }
            // Don't return, allow component to render mock state if we add it
            // return; 
        }
    ...
    const handleGetDirections = () => {
        ...
        if (!GOOGLE_MAPS_API_KEY) {
            ...
            setTimeout(() => {
                ...
                if (mapRef.current) {
                    mapRef.current.innerHTML = `
                        <div style="width:100%;height:100%;background:#222;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#888;text-align:center;border: 2px solid #4f46e5;">
                            <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#4f46e5" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mb-4"><polygon points="3 11 22 2 13 21 11 13 3 11"></polygon></svg>
                            <p class="font-bold text-white mb-2">Navigating to: ${destination}</p>
                            <p class="text-indigo-400">Turn right in 50 meters (Demo)</p>
                            <p>Map API Key missing - Simulation Active</p>
                        </div>
                    `;
                }
            }, 1000);
