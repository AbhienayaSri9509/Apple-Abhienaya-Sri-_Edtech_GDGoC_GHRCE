...
def text_to_sign(req: TextToSignRequest):
    ...
    try:
        if not os.path.exists(pkl_path):
           # Mock fallback for demo if dataset is missing
           print(f"WARNING: PKL file missing: {pkl_path}. Returning mock response.")
           # Return a dummy or placeholder video if available, or just success metadata
           # For now, let's return a success response but maybe with a flag or empty videoUrl if client can handle it,
           # OR better, generate a dummy video if possible.
           # Since we can't generate without data, we'll point to a placeholder.
           placeholder_video = "/videos/placeholder_sign.mp4" 
           # You might want to ensure this file exists or the frontend handles 404 gracefully.
           # But to stop the crash:
        ...
        print(f"Error processing text-to-sign: {e}")
        # Return a generic error or mock to keep the UI alive
        return TextToSignResponse(
             videoUrl="", 
             matchedSentence=sentence_text,
             sentenceName=sentence_name
